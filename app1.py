...
# Page config
st.set_page_config(
    page_title="Clinical Trials Intelligent Assistant",
    page_icon="🏥",
    layout="wide"
)
...
def display_header():
    ...
    with col1:
        st.title("Clinical Trials Intelligent Assistant")
        st.write("AI-Powered Clinical Trial Analysis System")
...
def display_graph_visualization(graph):
    ...
    try:
        ...
        st.image(mermaid_png, caption="System Architecture Visualization")
    except Exception as e:
...
def main():
    ...
    if st.session_state.nav == "Document Upload":
        ...
        if results := st.session_state.get("results"):
            tab1, tab2, tab3, tab4 = st.tabs(["Analysis", "Alerts", "Report", "Architecture"])
